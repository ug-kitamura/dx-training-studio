...
### Requirement: 入れ物直下の配信プレビュー用スクリプト

入れ物直下に配信プレビュー用スクリプト `preview-mandala-deploy.bat` を1本置かなければならない（SHALL）。起動スクリプト4本と同じく、自身の位置（`%~dp0`）を基準に対象ディレクトリを解決しなければならない（SHALL）。依存の整合性の確認には起動スクリプトと同じ番人（`mandala\node_modules\.bin\next.cmd`）を用いなければならない（SHALL）。

⚠ **これは起動スクリプト4本とは別枠である。**アプリを起動する道具ではなく、**配信成果物を検証する道具**であり、目的も寿命も異なる。「起動スクリプト4本」の要件に数えてはならない（SHALL NOT）。名前に `start-` を用いてはならない（SHALL NOT）——役割の違いを名前で示すため。

挙動の要件は `publishing-site-build` の「配信プレビュー用スクリプト」に従う。

#### Scenario: 入れ物直下から実行できる

- **WHEN** 入れ物直下で `preview-mandala-deploy.bat` を実行する
- **THEN** カレントディレクトリがどこであっても `mandala/` が正しく解決される

#### Scenario: 起動スクリプトと区別できる

- **WHEN** 入れ物直下のファイル一覧を見る
- **THEN** `start-` で始まる4本（アプリ起動）と、`preview-` で始まる1本（配信検証）が名前で区別できる

#### Scenario: 依存が壊れていても素通りしない

- **WHEN** `mandala/node_modules` は存在するが `node_modules\.bin` が失われた状態で実行する
- **THEN** 依存の入れ直しが実行されてから処理が続行される

### Requirement: 入れ物直下の `.bat` は ASCII・CRLF で書く

入れ物直下の `.bat` ファイルは、**コメントも含めて ASCII 文字だけで書かなければならない（SHALL）。改行は CRLF でなければならない（SHALL）。**日本語をコメント・メッセージのいずれにも入れてはならない（SHALL NOT）。

⚠ **cmd はバッチファイルをシステムの ANSI コードページ（日本語環境では CP932）で読む。**UTF-8 で書いた日本語は化け、Shift-JIS として解釈された結果に `\` などの制御的な文字が現れる。その結果 **`REM` 行がコマンドとして実行され、近くの `set` 行が壊れる**。実測では、日本語コメントを含む `.bat` でポート番号の変数が空になり、配信サーバーが引数不足で起動に失敗した——**バッチが「動くが挙動がおかしい」形で壊れる**ため、原因に辿り着きにくい。

既存の起動スクリプト4本がいずれも英語のみで書かれているのは、この理由による（暗黙の規約を明文化したもの）。**日本語で残したい説明は spec・README・handoff に書く。**

#### Scenario: 入れ物直下の bat が ASCII である

- **WHEN** 入れ物直下の `.bat` を確認する
- **THEN** ASCII 以外のバイトを含まない
- **AND** 改行は CRLF である

#### Scenario: 日本語コメントを入れると壊れる

- **WHEN** 日本語コメントを含む `.bat` を実行する
- **THEN** コメント行がコマンドとして実行され、変数の設定が壊れる（このため禁止する）
